...
ark_ai_voices_fnc_selectSound = {
    ...
    if (isNil "_soundPathArr") then {
        ...
        if (_dirPath isEqualTo "") exitWith {
            [COMPONENT_BEAUTIFIED, "ERROR", "fnc_selectSound", "No directory path for sounds found for", _speaker] call ark_admin_tools_fnc_log;
            _unit setVariable ["ark_ai_voices_var_disableVoice", true];
        };
        ...
        if (_protocolArr isEqualTo []) exitWith {
            [COMPONENT_BEAUTIFIED, "ERROR", "fnc_selectSound", "No protocol path sound files available for", [_speaker, _voiceLine]] call ark_admin_tools_fnc_log;
            _unit setVariable ["ark_ai_voices_var_disableVoice", true];
        };
    ...
    if (isNil "_soundPathArr" || { _soundPathArr isEqualTo [] }) exitWith {
        [COMPONENT_BEAUTIFIED, "ERROR", "fnc_selectSound", "No sound file to play for unit", _unit] call ark_admin_tools_fnc_log;
        _unit setVariable ["ark_ai_voices_var_disableVoice", true];
    };

ark_chase_ai_fnc_init = {
    ...
    if (_syncdTrg isEqualTo []) exitWith {
        [COMPONENT_BEAUTIFIED, "ERROR","fnc_init","Trigger not syncd to the module"] call ark_admin_tools_fnc_log;
    };

    if (count _syncdTrg > 1) then {
        [COMPONENT_BEAUTIFIED, "WARNING","fnc_init","Only sync one trigger to the module"] call ark_admin_tools_fnc_log;
    };

    private _unitTemplate = _logic getVariable ["Unit_Template", "ADMIRAL"];
    if (_unitTemplate isEqualTo "") exitWith {
        [COMPONENT_BEAUTIFIED, "ERROR","fnc_init","Blank unit template provided!"] call ark_admin_tools_fnc_log;
    };
    if (_unitTemplate isEqualTo "ADMIRAL") then {
        _unitTemplate = adm_camp_defaultUnitTemplate;
        [COMPONENT_BEAUTIFIED, "INFO","fnc_init","Using default unit template",_unitTemplate] call ark_admin_tools_fnc_log;
    };
    ...
    if (_unitClassNames isEqualTo "[ADMIRAL]") then {
        ...
        [COMPONENT_BEAUTIFIED, "INFO","fnc_init","Using default unit classnames",_unitClassNames] call ark_admin_tools_fnc_log;
    } else {
        _unitClassNames = call compile (_unitClassNames);
        if (!((typeName _unitClassNames) isEqualTo "ARRAY")) exitWith {
            [COMPONENT_BEAUTIFIED, "ERROR","fnc_init","Improper classname formatting provided, must be an ARRAY!",_unitClassNames] call ark_chase_ai_fnc_log;
        };
        [COMPONENT_BEAUTIFIED, "INFO","fnc_init","Using custom unit classnames",_unitClassNames] call ark_admin_tools_fnc_log;
    };
    ...
    ark_chase_ai_var_wpAccuracy = _logic getVariable ["Waypoint_Accuracy", 5];

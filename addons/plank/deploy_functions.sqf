...
plank_deploy_fnc_setFortVariables = {
    params ["_unit","_fortIndex","_fort","_horizontalOffset","_relativeHeight","_direction","_distance","_pitch","_bank","_heightMode","_lockMode"];
...
plank_deploy_fnc_addPlankAction = {
    ...
    if (isNil {_unit getVariable "plank_deploy_plankActionId"}) then {
        private _plankActionId = _unit addAction ['<t color="#3748E3">Plank</t>', ADDON_PATH(plank_action.sqf), [], 100, false, false, "", "driver _target == _this"];
...
plank_deploy_fnc_removePlankAction = {
    ...
    private _actionId = _unit getVariable "plank_deploy_plankActionId";
...
plank_deploy_fnc_tryStartFortPlacement = {
    ...
    private _currentFortIndex = _unit getVariable ["plank_deploy_fortIndex", DEFAULT_FORT_INDEX];
...
plank_deploy_fnc_pickupObject = {
    ...
    private _placedByData = _object getVariable ["plank_deploy_placedByData", [objNull, 0]];
...
plank_deploy_fnc_getFortifications = {
    ...
    for "_i" from 1 to (count FORTS_DATA) - 1 do {
        private _fortNameAndIndex = [FORTS_DATA select _i select 0, _i];
...
plank_deploy_fnc_addNewFortification = {
    ...
    private _index = count FORTS_DATA;
    private _fortCounts = _unit getVariable ["plank_deploy_fortCounts", [1]];
...
plank_deploy_fnc_addNewDefaultFortificationWithDistance = {
    params ["_unit","_className","_distance","_displayName"];

    private _actualDisplayName = _displayName;
...
plank_deploy_fnc_removeFortification = {
    ...
    if (_fortIndex > 0 && {_fortIndex < count FORTS_DATA} && {_fortIndex < count _fortCounts} && {_fortCounts select _fortIndex > 0}) then {
        private _newCount = ((_fortCounts select _fortIndex) - _count) max 0;

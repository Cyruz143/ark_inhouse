 ...
 * N/A
 *
 * Locality: Server (called from preInit)
 *
 * Arguments:
 * None
 *
...
private _factionConfigs = "ts_camouflage in getArray (_x >> 'camouflage') && {getText (_x >> 'name') isNotEqualTo 'Default'}" configClasses (configFile >> "Hull3" >> "Faction");
private _factionConfig = _factionConfigs select floor random count _factionConfigs;

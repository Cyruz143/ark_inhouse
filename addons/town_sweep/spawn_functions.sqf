...
ts_spawn_fnc_onAdmiralInit = {
    private ["_templateConfigs", "_templateConfig"];
    _templateConfigs = "ts_camouflage in getArray (_x >> 'camouflage') && {configName _x != 'Base'} && {getText (_x >> 'side') != ts_player_factionRawSide}" configClasses (ADMIRAL_CONFIG_FILE >> "UnitTemplates");
    _templateConfig = _templateConfigs select floor random count _templateConfigs;
...
ts_spawn_fnc_selectRandomLocation = {
    private ["_locationEntry", "_location"];
    _locationEntry = ts_spawn_locaionEntries select floor random count ts_spawn_locaionEntries;
    ts_spawn_selectedLocation =
        [ getText (_locationEntry >> "name")
        , getArray (_locationEntry >> "position")
        , (((getNumber (_locationEntry >> "radiusA")) max (getNumber (_locationEntry >> "radiusB"))) * 1.5) max 800
        , false
        ];
...
ts_spawn_fnc_changeLocationSize = {
    ...
    private _size = (ts_spawn_selectedLocation select 2) + _sizeChange;
    ts_spawn_selectedLocation set [2, _size];

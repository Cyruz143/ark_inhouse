...
adm_camp_fnc_placeMan = {
    ...
    LOG_5("admiral.camp.create: Spawned unit %1 at position %2, in group %3 with type %4 and classname %5.",_unit,_position,_group,_unitType,typeOf _unit);
...
adm_camp_fnc_spawnCrew = {
    ...
    LOG_4("admiral.camp.create: Spawned crew %1 for vehicle %2, in group %3 with type %4.",_crew,_vehicle,_group,_unitType);
...
adm_camp_fnc_tryAddPaths = {
...

    {
        ...
        if ([_pathPositions select 0, GET_ZONE_AREA(_zone), GET_ZONE_POSITION(_zone)] call adm_common_fnc_isPositionInArea) then {
            ...
            PUSH(_zonePaths,_x);
            LOG_2("admiral.camp.create: Path %1 is inside Camp Zone %2. Adding it to zone paths.",_x,GET_ZONE_ID(_zone));
        };
    } forEach _paths;
};
...
adm_camp_fnc_getLogicEndTrigger = {
    ...
    if (count _trigger == 0) then {
        ...
        LOG_2("admiral.camp.create: No end trigger was found. Created new end trigger %1 at position %2.",_trigger,_wpPos);
    } else {
...
adm_camp_fnc_createPatrolWaypoints = {
    ...
    LOG_2("admiral.camp.create: Created %1 patrol waypoint(s) for group %2.",_noOfWaypoints,_group);
};
...
adm_camp_fnc_createCampWaypoints = {
    ...
    LOG_3("admiral.camp.create: Created %1 path waypoint(s) for group %2 using path %3.",count _pathPositions,_group,_path);
};
...
adm_camp_fnc_disableCamp = {
    ...
    INFO_1("Admiral Camp Create: Camp Zone %1 has been disabled.",GET_ZONE_ID(_zone));
};
...
adm_camp_fnc_spawnInfGroup = {
    ...
    LOG_4("admiral.camp.create: Spawned %1 unit(s) for group %2 of type %3 in Zone %4.",_groupSize,_group,GROUP_TYPE_ARRAY select _groupType,GET_ZONE_ID(_zone));
...
adm_camp_fnc_spawnVehicleGroup = {
    ...
    LOG_4("admiral.camp.create: Spawned crew for vehicle %1 for group %2 of type %3 in Zone %4.",_vehicle,_group,GROUP_TYPE_ARRAY select _groupType,GET_ZONE_ID(_zone));
    [format ["%1.spawned.group", GET_ZONE_TYPE(_zone)], [_group, GROUP_TYPE_ARRAY select _groupType, _zone]] call adm_event_fnc_emitEvent;
    ["zone.spawned.group", [_group, GROUP_TYPE_ARRAY select _groupType, _zone]] call adm_event_fnc_emitEvent;
    ...
    [
        {
            ...
            private _spawnedInfGroups = [];
            _spawnedInfGroups = [_zone, GROUP_TYPE_INF, adm_camp_fnc_periodicCanSpawnGroups, adm_camp_fnc_periodicSpawnInfGroups] call adm_camp_fnc_trySpawnGroups;
            _zoneInfGroups append _spawnedInfGroups;

            private _spawnedTechGroups = [];
            _spawnedTechGroups = [_zone, GROUP_TYPE_TECH, adm_camp_fnc_periodicCanSpawnGroups, adm_camp_fnc_periodicSpawnTechGroups] call adm_camp_fnc_trySpawnGroups;
            _zoneTechGroups append _spawnedTechGroups;

            private _spawnedArmourGroups = [];
            ...
            private _allGroups = (count _spawnedInfGroups) + (count _spawnedTechGroups) + (count _spawnedArmourGroups);

            if (_allGroups > 0) then {
                ...
                INFO_1("Admiral Camp: Periodic Camp Zone %1 not required to spawn.",GET_ZONE_ID(_zone));
            };
            ...
            if ([_zone] call adm_camp_fnc_isPoolEmpty || {!IS_CAMP_ENABLED(_zone)}) exitWith {
                INFO_1("Admiral Camp: Periodic Camp Zone %1 has stopped.",GET_ZONE_ID(_zone));
    ...
    [
        {
            ...
            private _spawnedInfGroups = [];
            _spawnedInfGroups = [_zone, GROUP_TYPE_INF, adm_camp_fnc_onDemandCanSpawnGroups, adm_camp_fnc_onDemandSpawnInfGroups] call adm_camp_fnc_trySpawnGroups;
            _zoneInfGroups append _spawnedInfGroups;

            private _spawnedTechGroups = [];
            _spawnedTechGroups = [_zone, GROUP_TYPE_TECH, adm_camp_fnc_onDemandCanSpawnGroups, adm_camp_fnc_onDemandSpawnTechGroups] call adm_camp_fnc_trySpawnGroups;
            _zoneTechGroups append _spawnedTechGroups;
            ...
            if (_allGroups > 0) then {
                ...
                INFO_1("Admiral Camp: On-demand Camp Zone %1 not required to spawn.",GET_ZONE_ID(_zone));
            };

            ["camp.spawned.groups", [_zoneInfGroups, _zoneTechGroups, _zoneArmourGroups, _zone]] call adm_event_fnc_emitEvent;

            if ([_zone] call adm_camp_fnc_isPoolEmpty || {!IS_CAMP_ENABLED(_zone)}) exitWith {
                INFO_1("Admiral Camp: On-demand Camp Zone %1 has stopped.",GET_ZONE_ID(_zone));
                _id call CBA_fnc_removePerFrameHandler;
            };
        },
        GET_CAMP_DELAY(_zone),
    ...
    [
        {
            ...
            private _spawnedInfGroups = [];
            _spawnedInfGroups = [_zone, GROUP_TYPE_INF, adm_camp_fnc_randomCanSpawnGroups, adm_camp_fnc_randomSpawnInfGroups] call adm_camp_fnc_trySpawnGroups;
            _zoneInfGroups append _spawnedInfGroups;

            private _spawnedTechGroups = [];
            _spawnedTechGroups = [_zone, GROUP_TYPE_TECH, adm_camp_fnc_randomCanSpawnGroups, adm_camp_fnc_randomSpawnTechGroups] call adm_camp_fnc_trySpawnGroups;
            _zoneTechGroups append _spawnedTechGroups;
            ...
            if (_allGroups > 0) then {
                ...
                INFO_1("Admiral Camp: Random Camp Zone %1 not required to spawn.",GET_ZONE_ID(_zone));
            };

            ["camp.spawned.groups", [_zoneInfGroups, _zoneTechGroups, _zoneArmourGroups, _zone]] call adm_event_fnc_emitEvent;

            if ([_zone] call adm_camp_fnc_isPoolEmpty || {!IS_CAMP_ENABLED(_zone)}) exitWith {
                INFO_1("Admiral Camp: Random Camp Zone %1 has stopped.",GET_ZONE_ID(_zone));
                _id call CBA_fnc_removePerFrameHandler;
            };
        },
        GET_CAMP_DELAY(_zone),
...
adm_camp_setGroupDelay = {
    ...
    LOG_1("admiral.camp: Multiplied variable 'adm_camp_groupDelay' with variable 'adm_camp_campDelay' in Camp Zone %1.",GET_ZONE_ID(_zone));
};
...
adm_camp_fnc_initZone = {
    ...
    INFO_1("Admiral Camp: Camp Zone %1 has been succesfully initialized.",GET_ZONE_ID(_zone));

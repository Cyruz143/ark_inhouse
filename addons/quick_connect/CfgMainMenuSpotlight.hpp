class CfgMainMenuSpotlight {
    class GVAR(joinServer) {
        text = "Join ARK server";
        textIsQuote = 0;
        picture = QPATHTOF(resource\button.paa);
        action = QUOTE([ARR_2('arma.ark-group.org',2320)] call (uiNamespace getVariable QQFUNC(joinServer)));
        actionText = "Sessions are Saturdays from 7pm (UK TIME)";
        condition = "true";
    };

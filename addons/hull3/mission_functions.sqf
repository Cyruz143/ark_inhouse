...
hull3_mission_fnc_serverInit = {
    ...
    LOG("hull3.mission: Server init finished.");
};
...
hull3_mission_fnc_readMissionParamValues = {
    if (!isNil {hull3_mission_date}) then {
        ...
        LOG_1("hull3.mission.params: Mission param 'hull3_mission_date' was set to %1.",hull3_mission_date);
    };
    if (!isNil {hull3_mission_timeOfDay}) then {
        ...
        LOG_1("hull3.mission.params: Mission param 'hull3_mission_timeOfDay' was set to %1.",hull3_mission_timeOfDay);
    };
    if (!isNil {hull3_mission_fog}) then {
        ...
        LOG_1("hull3.mission.params: Mission param 'hull3_mission_fog' was set to %1.",hull3_mission_fog);
    };
    if (!isNil {hull3_mission_weather}) then {
        ...
        LOG_1("hull3.mission.params: Mission param 'hull3_mission_weather' was set to %1.",hull3_mission_weather);
    };
};
...
hull3_mission_fnc_getDateTime = {
    if (isNil {hull3_mission_date}) then {
        ...
        LOG_1("hull3.mission.datetime: Mission param 'hull3_mission_date' was not set, using default %1.",hull3_mission_date);
    };
    if (isNil {hull3_mission_timeOfDay}) then {
        hull3_mission_timeOfDay = DEFAULT_TIME_OF_DAY;
        LOG_1("hull3.mission.datetime: Mission param 'hull3_mission_timeOfDay' was not set, using default %1.",hull3_mission_timeOfDay);
    };
...
hull3_mission_fnc_getFog = {
    if (isNil {hull3_mission_fog}) then {
        ...
        LOG_1("hull3.mission.fog: Mission param 'hull3_mission_fog' was not set, using default %1.",hull3_mission_fog);
    };
...
hull3_mission_fnc_getWeather = {
    if (isNil {hull3_mission_weather}) then {
        ...
        LOG_1("hull3.mission.weather: Mission param 'hull3_mission_weather' was not set, using default %1.",hull3_mission_weather);
    };
    if (hull3_mission_weather #0 == -1 && {isServer}) then {
        ...
        LOG_1("hull3.mission.weather: Random weather was selected. Generated random weather %1 for server.",hull3_mission_weather);
    } else {
        if (!isDedicated && !isServer) then {
            hull3_mission_weather = [0, 0, 0, 0, 0, 0, 0, 0];
            LOG_1("hull3.mission.weather: Random weather was selected. Using default weather %1 for client.",hull3_mission_weather);
        };
    };
...
hull3_mission_fnc_setWeather = {
    ...
    if (!isNil "_snow" && {_snow isEqualTo 1}) then {
        ...
        LOG_2("hull3.mission.weather: Snow weather set to %1 with snowData set to %2.",_snow,_snowData);
    };
};
...
hull3_mission_fnc_setEnviroment = {
    ...
    LOG_3("hull3.mission.weather: Environment was set. Date to %1, fog to %2 and weather to %3.",[] call hull3_mission_fnc_getDateTime,[] call hull3_mission_fnc_getFog,[] call hull3_mission_fnc_getWeather);
};
...
hull3_mission_fnc_addPlayerEHs = {
    ...
    LOG("hull3.mission.ehs: Player event handlers were added.");
};
...
hull3_mission_fnc_addServerEHs = {
    ...
    LOG("hull3.mission.ehs: Server event handlers were added.");
};
...
hull3_mission_fnc_serverSafetyTimerLoop = {
    if (!isNil {hull3_mission_safetyTimerEnd} && {hull3_mission_safetyTimerEnd > 0}) then {
        ...
        LOG_1("hull3.mission.safetytimer: Safety timer has been initialized with value %1.",hull3_mission_safetyTimer);
        while {(hull3_mission_safetyTimer #1) < hull3_mission_safetyTimerEnd && {!hull3_mission_safetyTimerAbort}} do {
            ...
            LOG_1("hull3.mission.safetytimer: Safety timer has been published to clients with value %1.",hull3_mission_safetyTimer);
            if (!isDedicated) then {
...
hull3_mission_fnc_serverSafetyTimerCountDown = {
    ...
    if (_isAborted) then {
        ...
        LOG_1("hull3.mission.safetytimer: Safety timer has been aborted. Starting countdown from %1 seconds.",hull3_mission_safetyTimer #1);
        for "_i" from _countDownLength to 0 step -1 do {
            hull3_mission_safetyTimer set [1, _i];
            publicVariable "hull3_mission_safetyTimer";
            LOG_1("hull3.mission.safetytimer: Safety timer has been published to clients with countdown time at %1 seconds.",hull3_mission_safetyTimer #1);
            if (!isDedicated) then {
                hull3_mission_safetyTimer call hull3_mission_fnc_handleSafetyTimeChange;
            };
            sleep 1;
...
hull3_mission_fnc_clientSafetyTimerLoop = {
    if (!isNil {hull3_mission_safetyTimerEnd} && {hull3_mission_safetyTimerEnd > 0}) then {
        ...
        LOG("hull3.mission.safetytimer: Starting safety timer loop.");
        [{
            params ["_args", "_id"];
            ...
            if ([] call hull3_mission_fnc_hasSafetyTimerEnded) then {
                ...
                LOG("hull3.mission.safetytimer: Safety timer has ended. Removed fired EH.");
            };
...
hull3_mission_fnc_receiveJipSync = {
    ...
    LOG_2("hull3.mission.jip: Received JIP sync %1 from server for client %2.",owner player,_this);
